...
class GPT(nn.Module):
    ...
    def generate(self, idx, device, max_new_tokens, temperature=1.0, do_sample=False, top_k=None):
        ...
        for _ in range(max_new_tokens):
            ...
            idx_n = idx_next.squeeze()
            prob.append(probs[0, idx_n].cpu())
            # append sampled index to the running sequence and continue
            idx = torch.cat((idx, idx_next), dim=1)

            prob_6, idx_6 = torch.topk(probs, k=6, dim=-1)
            prob_l.append(prob_6.cpu())
            idx_l.append(idx_6.cpu())

        return idx, prob, idx_l, prob_l
